...
public class ThreeSum {
    ...
    public static List<List<Integer>> solution4(int[] nums) {
        ...
        Arrays.sort(nums);
        Set<List<Integer>> combinations = ArrayUtil.combine(nums, 3);
    ...
    public static List<List<Integer>> solution5(int[] nums) {
        ...
        HashSet<Integer> set1 = new HashSet<Integer>(); // 存放无重复的元素
        HashSet<Integer> set2 = new HashSet<Integer>(); // 与set1中重复的元素
        for (int i = 0; i < nums.length; i++) {
    ...
    public static List<List<Integer>> solution6(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            int target = 0 - nums[i];
            int[] subSums = Arrays.copyOfRange(nums, i+1, nums.length);
            Set<int[]> twoSums = twoSum(subSums, target);
            if (twoSums.size() > 0) {
    ...
    private static Set<int[]> twoSum(int[] nums, int target) {
        Set<int[]> set = new HashSet<>();
        Set<Integer> set1 = new HashSet<>();
        for (int i = 0; i < nums.length; i++) {
            int complement = target - nums[i];
            if (set1.contains(complement)) {
                set.add(new int[] { Math.min(nums[i], complement), Math.max(nums[i], complement) });
            }
            set1.add(nums[i]);
        }
        return set;
    ...
    // 转换为two sum，简化版
    public static List<List<Integer>> solution7(int[] nums) {
        List<List<Integer>> lists = new ArrayList<>();
        if (nums == null || nums.length < 3) {
            return lists;
        }
    ...
    // 转化为two sum
    public static List<List<Integer>> solution8(int[] nums) {
        List<List<Integer>> lists = new ArrayList<>();
        if (nums == null || nums.length < 3) {
            return lists;
        }
        HashSet<List<Integer>> sets = new HashSet<>();
        // 除去3个0，不可能选择一个数字超过3次。可先对数组进行处理，最多保留两个相同的数，或者3个0
        int countZero = 0;
        HashSet<Integer> set2 = new HashSet<>(); // 存放无重复的元素
        HashSet<Integer> set3 = new HashSet<>(); // 与set2中重复的元素
        for (int i = 0; i < nums.length; i++) {
        ...
        List<Integer> list2 = new ArrayList<>(); //nums 优化后的list, 删除超过两次的元素。
        list2.addAll(set2);
        list2.addAll(set3);
        for (int i = 0; i < list2.size(); i++) {
            int target = 0 - list2.get(i);
            Set<Integer> set1 = new HashSet<Integer>();
            for (int j = i+1; j < list2.size(); j++) {
                int complement = target - list2.get(j);
                if (set1.contains(complement)) {
                    List<Integer> list1 = new ArrayList<Integer>();
    ...
    // 分组，正数一组，负数一组。
    public static List<List<Integer>> solution9(int[] nums) {
        List<List<Integer>> lists = new ArrayList<>();
        if (nums == null || nums.length < 3) {
            return lists;
        }
        ...
        for (int i = 0; i < list3.size(); i++) {
            Set<int[]> twoSums = twoSum(list4.stream().mapToInt(j->j).toArray(),  0 - list3.get(i));
            if (twoSums.size() > 0) {
                for (int[] twoSum : twoSums) {
                    List<Integer> list1 = new ArrayList<>();
                    list1.add(Math.min(twoSum[0], twoSum[1]));
                    list1.add(Math.max(twoSum[0], twoSum[1]));
        ...
        for (int i = 0; i < list4.size(); i++) {
            Set<int[]> twoSums = twoSum(list3.stream().mapToInt(j->j).toArray(),  0 - list4.get(i));
            if (twoSums.size() > 0) {
                for (int[] twoSum : twoSums) {
                    List<Integer> list1 = new ArrayList<>();
                    list1.add(Math.min(twoSum[0], twoSum[1]));
    ...
    public static List<List<Integer>> solution10(int[] nums) {
        ...
        if (list2.size() >= 3) {
            // the first 3 numbers
            List<Integer> list1 = new ArrayList<>();
            if (list2.get(0) + list2.get(1) + list2.get(2) == 0) {
    ...
    // 排序后，固定一个点，转换为two sum问题，two point算法。
    public static List<List<Integer>> solution11(int[] nums) {
        ...
        for (int i = 0; i < nums.length-2; i++) {
            ...
            // 排序后两端扫一遍, two point算法。
            int leftIndex = i+1;
            int rightIndex = nums.length-1;
            while (leftIndex < rightIndex) {
                if (nums[leftIndex] + nums[rightIndex] == twoSumTarget) {
                    res.add(Arrays.asList(nums[i], nums[leftIndex], nums[rightIndex]));
                    // 继续找，去重，需要将leftIndex和rightIndex移动到他们下一个不相等的地方。
                    int newLeftIndex = leftIndex+1;
                    while (newLeftIndex < rightIndex) {
                        if (nums[newLeftIndex] != nums[leftIndex]) {
                            break;
                        } else {
                            newLeftIndex++;
                        }
                    ...
                    while (newRightIndex > leftIndex) {
                        if (nums[newRightIndex] != nums[rightIndex]) {
                            break;
                        } else {
                            newRightIndex--;
                        }
                ...
                } else if (nums[leftIndex] + nums[rightIndex] < twoSumTarget) {
                    leftIndex++;
                } else {
                    rightIndex--;
                }
